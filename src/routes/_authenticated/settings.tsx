  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Account settings | CertKeep" },
      {
        ...
        content: "Manage your CertKeep account and workspace details.",
      },
...
function AccountPage() {
  ...
  const mutation = useMutation({
    mutationFn: (input: { fullName: string; companyName: string }) => save({ data: input }),
    onSuccess: () => {
      setSaved(true);
  ...
  return (
    <AppShell title="Settings" subtitle="Manage your account and workspace">
      <Panel className="overflow-hidden">
        <SettingsNav />
        {isLoading ? (
          <p className="p-6 text-[12px] text-muted-foreground">Loading account…</p>
        ) : (
          <form onSubmit={submit} className="max-w-[720px] space-y-5 p-6">
            <div>
              <h2 className="text-[17px]">Account details</h2>
              <p className="mt-1 text-[11px] text-muted-foreground">
                These details identify you and your company inside CertKeep.
              </p>
            </div>
            ...
            <Field label="Sign-in email" htmlFor="email">
              <input id="email" value={data?.email ?? ""} readOnly className={inputClass} />
            </Field>
            <div className="flex flex-wrap items-center gap-3">
              ...
              {mutation.isError ? (
                <span className="text-[11px] text-destructive">Could not save changes.</span>
              ) : null}
            </div>
          </form>
        )}
