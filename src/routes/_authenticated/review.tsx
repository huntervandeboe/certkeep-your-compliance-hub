...
export const Route = createFileRoute("/_authenticated/review")({
  ...
  head: () => ({
    meta: [
      { title: "Fast document review | CertKeep" },
      {
        name: "description",
        content: "Review subcontractor documents beside project requirements.",
      },
      { property: "og:title", content: "Fast document review | CertKeep" },
      {
        property: "og:description",
        content: "Review and resolve uploaded compliance documents quickly.",
      },
...
function ReviewPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    open({ data: { id: selected.id } })
      .then((result) => {
        if (active) setDocumentUrl(result.url);
      })
      .catch(() => {
        if (active) setDocumentUrl("");
      });
    return () => {
      active = false;
    };
  ...
  const requirement =
    data?.requirements.find((item) => item.id === selected?.requirement_id) ??
    data?.requirements.find(
      (item) =>
        item.project_id === selected?.project_id && item.document_type === selected?.doc_type,
    );

  return (
    <AppShell
      title="Document review"
      subtitle={`${queue.length} submission${queue.length === 1 ? "" : "s"} waiting for a decision`}
    >
      {isLoading ? (
        ...
        <div className="grid min-h-[660px] overflow-hidden rounded-[18px] border border-border bg-surface shadow-sm xl:grid-cols-[280px_minmax(360px,1fr)_320px]">
          <aside className="border-b border-border xl:border-b-0 xl:border-r">
            <div className="border-b border-border px-4 py-4">
              <p className="text-[11px] font-bold tracking-[.1em] text-muted-foreground uppercase">
                Review queue
              </p>
            </div>
            <div className="max-h-[220px] overflow-auto xl:max-h-[600px]">
              {queue.map((item) => {
                const company =
                  data?.subcontractors.find((s) => s.id === item.subcontractor_id)?.company ??
                  "Subcontractor";
                return (
                  <button
                    key={item.id}
                    ...
                    className={`flex w-full items-center gap-3 border-b border-border px-4 py-4 text-left ${item.id === selected.id ? "bg-warning-soft" : "hover:bg-surface-muted"}`}
                  >
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-surface-muted text-ink">
          ...
          <section className="flex min-h-[460px] flex-col bg-surface-muted/50">
            <div className="flex items-center justify-between border-b border-border bg-surface px-5 py-3">
              <div>
                <p className="text-[12px] font-bold text-ink">
                  {selected.file_path ? "Submitted document" : "Document unavailable"}
                </p>
              ...
              {documentUrl ? (
                <a href={documentUrl} target="_blank" rel="noreferrer" className={btn.ghost}>
                  Open full size
                </a>
              ) : null}
            </div>
            {documentUrl ? (
              <iframe
                title={`${selected.doc_type} preview`}
                src={documentUrl}
                className="min-h-[520px] w-full flex-1 bg-surface"
              />
            ) : (
              <div className="grid flex-1 place-items-center text-center">
                <div>
                  <FileText className="mx-auto h-10 w-10 text-muted-foreground" />
                  <p className="mt-3 text-[12px] font-bold text-ink">Preview unavailable</p>
            ...
            <div className="space-y-5 p-5">
              <div>
                <div className="flex items-center justify-between">
                  <StatusBadge kind="submitted" />
                  <span className="text-[9px] font-bold text-muted-foreground">
                    {queue.findIndex((r) => r.id === selected.id) + 1} OF {queue.length}
                  </span>
                </div>
                <h2 className="mt-4 text-[18px]">{selected.doc_type}</h2>
                <p className="mt-1 text-[12px] text-muted-foreground">{sub?.company}</p>
              </div>
              <dl className="grid gap-3 border-y border-border py-4 text-[11px]">
                ...
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="text-right font-bold text-ink">
                    {selected.expiration_date ?? "Not provided"}
                  </dd>
                </div>
                <div className="flex justify-between gap-4">
              ...
              <div>
                ...
                {requirement ? (
                  <div className="mt-3 space-y-2 text-[11px]">
                    <p className="font-bold text-ink">{requirement.name}</p>
                    <p className="text-muted-foreground">Type: {requirement.document_type}</p>
                    <p className="text-muted-foreground">
                      Trade: {requirement.trade || "All trades"}
                    </p>
                    <p className="text-muted-foreground">
                      Warning window: {requirement.expiration_warning_days} days
                    </p>
                  </div>
                ) : (
                  <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
                    No project-specific requirement is linked. Review against your company’s
                    standard.
                  </p>
                )}
              </div>
              {correcting ? (
                <div className="space-y-3">
                  <label className="text-[11px] font-bold text-ink" htmlFor="correction">
                    Correction needed
                  </label>
                  <textarea
                    id="correction"
                  ...
                  <button
                    ...
                    onClick={() => mutation.mutate({ id: selected.id, action: "reject", reason })}
                  >
                ...
                <div className="grid gap-2">
                  <button
                    ...
                    onClick={() =>
                      mutation.mutate({ id: selected.id, action: "approve", reason: "" })
                    }
                  >
